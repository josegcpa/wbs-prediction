...
if __name__ == "__main__":
    ...
    for fold in range(args.n_folds):
        ...
        def param_generator_1():
            for network in all_networks:
                for p in network.parameters():
                    yield p
        def param_generator_2():
        ...
        schedule_1 = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer_1,factor=0.8,min_lr=0.000001)
        schedule_2 = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer_2,factor=0.8,min_lr=0.000001)
        ...
        while True:
            ...
            if S % 50 == 0:
                ...
                schedule_1.step(np.mean(L))
                schedule_2.step(np.mean(L))
                print(S,L,"(current learning rates=[{},{}])".format(schedule_1._last_lr,
                                                                    schedule_2._last_lr))
                network.train(True)
                for final_layer in all_lists['final_layers']:
