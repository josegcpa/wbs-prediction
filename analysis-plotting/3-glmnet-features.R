...
rbc_counts <-  rbind(
  read_csv(
    ...
    cbind(dataset = "MLLC"),
  read_csv(
    "datasets/rbc_counts_adden_1.csv",
    col_names = c("slide_id","counts"),
    col_types = c(col_character(),col_double()))  %>%
    cbind(dataset = "AC1"),
  read_csv(
    "datasets/rbc_counts_adden_2.csv",
    col_names = c("slide_id","counts"),
    col_types = c(col_character(),col_double())) %>%
    cbind(dataset = "AC2")
) %>%
...
wbc_counts <-  rbind(
  read_csv(
    ...
    cbind(dataset = "MLLC"),
  read_csv(
    "datasets/wbc_counts_adden_1.csv",
    col_names = c("slide_id","counts"),
    col_types = c(col_character(),col_double()))  %>%
    cbind(dataset = "AC1"),
  read_csv(
    "datasets/wbc_counts_adden_2.csv",
    col_names = c("slide_id","counts"),
    col_types = c(col_character(),col_double())) %>%
    cbind(dataset = "AC2")
) %>%
...
for (ID in IDs) {
  ...
  training_y_task <- label_conversion[[ID]][
    full_dataset_collection$fine_labels
  ]
  y_na <- is.na(training_y_task)
  training_y_task <- training_y_task[!y_na]
  X <- as.matrix(wbc_rbc_ratios[!y_na,])
  testing_X <- as.matrix(wbc_rbc_ratios_test)
  pp <- preProcess(X,method = c("center","scale","corr","nzv"))
  X <- predict(pp,X)
  testing_X <- predict(pp,testing_X)
  testing_y <- label_conversion[[ID]][
    full_dataset_collection_oos$fine_labels
    ]
  y_testing_na <- is.na(testing_y)
  testing_y <- testing_y[!y_testing_na]
  testing_X <- testing_X[!y_testing_na,]
  cw <- make_class_weights(training_y_task)
  alphas <- c(0.01,0.1,0.25,0.5,0.75,0.9,0.99)
  models <- list()
  for (alpha in alphas) {
    m <- cv.glmnet(
      X,training_y_task,family = "binomial",type.measure = "auc",
      nfolds = 5,weights = cw,alpha = alpha)
    l <- m$lambda
    l.min <- m$lambda.min
  ...
  test_proba_pred <- predict(trained_model,
                             newx = as.matrix(testing_X),type = "response")
  test_class_pred <- predict(trained_model,
                             newx = as.matrix(testing_X),type = "class")
  
  test_y <- factor(reverse_label_conversion[[ID]][as.numeric(test_class_pred)+1],
                   levels = reverse_label_conversion[[ID]])
  test_y_pred_class <- factor(reverse_label_conversion[[ID]][testing_y+1],
                              levels = reverse_label_conversion[[ID]])

  glmnet_models[[ID]]$model <- trained_model
  glmnet_models[[ID]]$metrics <- list(
    Training = list(
      ConfusionMatrix = confusionMatrix(table(
        reverse_label_conversion[[ID]][as.numeric(train_class_pred)+1],
        reverse_label_conversion[[ID]][training_y_task+1],
        dnn=reverse_label_conversion[[ID]])),
      AUC = roc(reverse_label_conversion[[ID]][training_y_task+1],
                train_proba_pred[,1],quiet = T,
                levels = reverse_label_conversion[[ID]])),
    Validation = list(
      ConfusionMatrix = confusionMatrix(test_y,test_y_pred_class),
...
all_auc <- list()
for (ID in IDs) {
  all_auc[[ID]] <- data.frame(
    task = ID,value = glmnet_models[[ID]]$metrics$Validation$AUC$auc)
}

do.call(all_auc,what = rbind)
...
features_to_keep <- which(
  dataset_AUC_values < 0.65 & dataset_AUC_values > 0.35)
...
all_roc_df %>%
  ...
  geom_linerange(data = validation_aucs_df,position = position_dodge(width = 0.9),
                 aes(ymin = auc_value - 1/sqrt(N),
                     ymax = ifelse(auc_value + 1/sqrt(N),1,auc_value + 1/sqrt(N))),
